...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.APOLLO.N_CTX
        ctx_init = cfg.TRAINER.APOLLO.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        # Default is 1, which is compound shallow prompting
        assert cfg.TRAINER.APOLLO.PROMPT_DEPTH >= 1, "For APOLLO, PROMPT_DEPTH should be >= 1"
        self.compound_prompts_depth = cfg.TRAINER.APOLLO.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        ...
        print('APOLLO design: Multi-modal Prompt Learning')
        print(f'Initial context: "{prompt_prefix}"')
        print(f"Number of APOLLO context words (tokens): {n_ctx}")
        ...
        # Minimum can be 1, which defaults to shallow APOLLO
        # compound prompts
...
class APOLLO(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.APOLLO.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        self.scaler = GradScaler() if cfg.TRAINER.APOLLO.PREC == "amp" else None
