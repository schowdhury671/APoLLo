...
class ResidualAttentionBlock_APOLLO(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['APOLLO_length']
        if i == 0:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'APOLLO':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_APOLLO(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer_APOLLO(nn.Module):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "APOLLO":
                self.visual = VisionTransformer_APOLLO(
                    input_resolution=image_resolution,
